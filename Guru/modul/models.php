...
if (isset($_POST['mapelSave'])) {
	$sql = mysqli_query($con, "INSERT INTO tb_roleguru VALUES (NULL,'$_POST[id_guru]','$_POST[kelas]','$_POST[mapel]') ") or die(mysqli_error($con));
	if ($sql) {
		echo "
...
elseif (isset($_POST['perangkatUpload'])) {
	...
	$sql = mysqli_query($con, "UPDATE tb_perangkat SET judul='$_POST[judul]', isi_perangkat='$_POST[isi_perangkat]',id_jenisperangkat='$_POST[id_jenis]' WHERE id_perangkat='$_POST[ID]' ") or die(mysqli_error($con));
	if ($sql) {
		echo "
			<script type='text/javascript'>
			setTimeout(function () {
...
elseif (isset($_POST['ujianSave'])) {
	$sql = mysqli_query($con, "INSERT INTO ujian VALUES(NULL,'$_POST[judul]','$_POST[tgl]','$_POST[jam]:$_POST[menit]:00','$_POST[jumlah]','$_POST[acak]','0','$_POST[id_jenis]','$_POST[id_guru]','$_POST[id_mapel]' ) ");
	if ($sql) {
		echo "
			<script type='text/javascript'>
			setTimeout(function () {
...
elseif (isset($_POST['kelasujianSave'])) {
	...
	if ($kls != NULL) {
		...
		for ($x = 0; $x < $jumlahTerpilih; $x++) {
			...
			$s = mysqli_query($con, "INSERT INTO kelas_ujian(id_ujian,id_kelas,aktif) 

		VALUES('$_POST[id]','$kls[$x]','Y')") or die(mysqli_error($con));
			if ($s) {
				echo "
			<script>
...
elseif (isset($_POST['kelasujianEssaySave'])) {
	...
	for ($x = 0; $x < $jumlahTerpilih; $x++) {
		...
		$s = mysqli_query($con, "INSERT INTO kelas_ujianessay(id_ujianessay,id_kelas,aktif) 
...
VALUES('$_POST[id]', '$_POST[soal]', '$_POST[p1]', '$_POST[p2]', '$_POST[p3]', '$_POST[p4]', '$_POST[p5]', '$_POST[kunci]')");
	...
	$sql = mysqli_query($con, "INSERT INTO ujian_essay VALUES(NULL,'$_POST[judul]','$_POST[tgl]','$_POST[jumlah]','$_POST[essay]','$_POST[id_jenis]','$_POST[id_guru]','$_POST[id_mapel]')");
	if ($sql) {
		...
		echo "
			...
			window.setTimeout(function(){ 
			window.location.replace('?page=essay');
			} ,3000);   
		</script>";
	}
...
elseif (isset($_POST['tugasSave'])) {
	...
	$sql = mysqli_query($con, "INSERT INTO tb_tugas VALUES(NULL,'$_POST[id_jenis]','$_POST[judul]','$_POST[isi_tugas]','$_POST[tgl]','$_POST[waktu]','$_POST[jumlahanggota]','$_POST[id_guru]','$_POST[id_mapel]' ) ");
	if ($sql) {
		echo "
			<script type='text/javascript'>
			setTimeout(function () {
...
elseif (isset($_POST['tugasEdit'])) {
	$sql = mysqli_query($con, "UPDATE tb_tugas SET id_jenistugas='$_POST[id_jenis]',judul_tugas='$_POST[judul]',isi_tugas='$_POST[isi_tugas]',tanggal='$_POST[tgl]',waktu='$_POST[waktu]',id_mapel='$_POST[id_mapel]' WHERE id_tugas='$_POST[ID]' ");
	if ($sql) {
		echo "
			<script type='text/javascript'>
			setTimeout(function () {
...
elseif (isset($_POST['kelastugasSave'])) {
	...
	for ($x = 0; $x < $jumlahTerpilih; $x++) {
		...
		$s = mysqli_query($con, "INSERT INTO kelas_tugas(id_tugas,id_kelas,aktif) 
