...
mysqli_query($con, "UPDATE tb_siswa SET status='mengerjakan' WHERE  id_siswa='$_SESSION[id_siswa]' ");

$rujian = mysqli_fetch_array(mysqli_query($con, "SELECT * FROM ujian WHERE id_ujian='$_GET[ujian]'"));

if ($rujian['acak'] == 'acak') {
	$qsoal = mysqli_query($con, "SELECT * FROM soal WHERE id_ujian='$_GET[ujian]' ORDER BY rand() LIMIT $rujian[jml_soal]");
} else {
	$qsoal = mysqli_query($con, "SELECT * FROM soal WHERE id_ujian='$_GET[ujian]' ORDER BY id_soal LIMIT $rujian[jml_soal]");
}

$q2soal = mysqli_query($con, "SELECT * FROM soal WHERE id_ujian='$_GET[ujian]' ORDER BY id_soal");
...
$qnilai = mysqli_query($con, "SELECT * FROM nilai WHERE id_siswa='$_SESSION[id_siswa]' AND id_ujian='$_GET[ujian]'");
if (mysqli_num_rows($qnilai) < 1) {
	...
	mysqli_query($con, "INSERT INTO nilai SET id_siswa='$_SESSION[id_siswa]',id_ujian='$_GET[ujian]', acak_soal='$acak_soal', jawaban='$jawaban', sisa_waktu='$waktu',waktu_selesai='$waktuselesai'");

	$kls = $soalid;
	foreach ($kls as $kelas) {
		mysqli_query($con, "INSERT INTO analisis SET id_siswa='$_SESSION[id_siswa]',id_ujian='$_GET[ujian]', id_soal='$kelas', jawaban='0'");
	}
} else {
	...
	if ($mn < 0) {
		mysqli_query($con, "UPDATE nilai SET sisa_waktu = '00:00:01' WHERE id_siswa='$_SESSION[id_siswa]' AND id_ujian='$_GET[ujian]'");
	} else {
		mysqli_query($con, "UPDATE nilai SET sisa_waktu = '$hr:$mn:$sc' WHERE id_siswa='$_SESSION[id_siswa]' AND id_ujian='$_GET[ujian]'");
	}
}
...
for ($s = 0; $s < count($arr_soal); $s++) {
	$rsoal = mysqli_fetch_array(mysqli_query($con, "SELECT * FROM soal WHERE id_soal='$arr_soal[$s]'"));
